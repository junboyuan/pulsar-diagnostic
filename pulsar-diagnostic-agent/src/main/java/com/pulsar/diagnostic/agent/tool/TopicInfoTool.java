...
public class TopicInfoTool {
    ...
    public TopicInfoTool(PulsarAdminClient pulsarAdminClient) {
        this.pulsarAdminClient = pulsarAdminClient;
    }

    @Tool(description = "Get detailed information about a specific Pulsar topic")
    public String getTopicInfo(
            @ToolParam(description = "Full topic name in format: persistent://tenant/namespace/topic") String topicName) {
        log.info("Tool: Getting topic info for: {}", topicName);
        try {
            ...
            return "Error getting topic info: " + e.getMessage();
        }
    }

    @Tool(description = "Get statistics for a specific topic including message rates and throughput")
    ...
    public String listTopicsInNamespace(
            @ToolParam(description = "Namespace name in format: tenant/namespace") String namespace) {
    ...
    @Tool(description = "Get subscriptions for a specific topic")
    public String getTopicSubscriptions(
            @ToolParam(description = "Full topic name") String topicName) {
        log.info("Tool: Getting subscriptions for topic: {}", topicName);
        try {
            ...
            return "Error getting subscriptions: " + e.getMessage();
        }
    }

    @Tool(description = "Check if a topic has message backlog (unconsumed messages)")
    public String checkTopicBacklog(
            @ToolParam(description = "Full topic name") String topicName) {
        log.info("Tool: Checking backlog for topic: {}", topicName);
        try {
            TopicInfo topic = pulsarAdminClient.getTopicInfo(topicName);
            ...
            return "Error checking backlog: " + e.getMessage();
        }
    }
